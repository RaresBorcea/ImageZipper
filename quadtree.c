...
// Creeaza 4 fii pentru nodul curent din arbore, utilizand culorile calculate ca medie
void createSons(Tree tree, unsigned char blue1, unsigned char green1, unsigned char red1, unsigned char blue2, 
	unsigned char green2, unsigned char red2, unsigned char blue3, unsigned char green3, unsigned char red3, 
	unsigned char blue4, unsigned char green4, unsigned char red4, uint32_t area, int32_t *no) {
...
// Insereaza informatia din arbore in vectorul structura QuadTreeNode
...
// Elibereaza memoria ocupata de matrice
...
// Elibereaza memoria ocupata de arbori
...
// Calculeaza culorile medii pe matrice si apeleaza functia de creare a fiilor
	...
	for(i = x; i < x + size; i++) {
		for(j = y; j < y + size; j++) {
			score = score + (mred - image[i][j].red) * (mred - image[i][j].red) + 
				    (mblue - image[i][j].blue) * (mblue - image[i][j].blue) + 
				    (mgreen - image[i][j].green) * (mgreen - image[i][j].green);
		}
	}
...
// Gestioneaza crearea vectorului QuadTreeNode
...
// Creeaza arborele pe baza vectorului citit din fisierul comprimat
...
// Creeaza matricea RGB pe baza arborelui
...
// Salveaza fisierul PPM rezultat in urma cerintelor
// 2, 3 si a bonusului
...
// Suprapune arborii a doua imagini
void merge(int32_t *no, Tree tree, int x, int y, uint32_t size, Tree tree1, Tree tree2, 
	int x1, int y1, uint32_t size1, int x2, int y2, uint32_t size2) {
...
int main(int argc, char const *argv[]) {
	...
	if(strcmp("-c", argv[1]) == 0) {
		...
		// daca se efectueaza decompresia
		strcpy(file, argv[2]);
		strcpy(dest, argv[3]);
		decomp(file, dest);
	} else if(strcmp("-m", argv[1]) == 0) {
		//daca se efectueaza oglindirea
		int i;
	...
	if(strcmp("-o", argv[1]) == 0) {
		// daca se efectueaza suprapunerea
